...
char dev_present[MAX_XDMA_DEVICES];
...
DEVICE_ATTR(xdma_dev_instance, S_IRUGO, show_device_numbers, NULL);
 ...
 */
const struct file_operations sg_polled_fops = {
	.owner = THIS_MODULE,
...
const struct file_operations sg_interrupt_fops = {
	.owner = THIS_MODULE,
	.open = char_sgdma_open,
	.release = char_sgdma_close,
	.read = char_sgdma_read,
 ...
 */
const struct file_operations bypass_fops = {
	.owner = THIS_MODULE,
	.open = char_open,
	.release = char_close,
 ...
 */
const struct file_operations events_fops = {
	.owner = THIS_MODULE,
	.open = char_open,
	.release = char_close,
...
#else
inline u64 find_feature_id(const struct xdma_dev *lro)
{
	return 0;
}
#endif
...
#else
inline int engine_initialize(struct xdma_dev *lro, int interrupts_offset)
{
	return 0;
}
#endif
...
#else
inline int engine_version(struct xdma_dev *lro, int engine_offset)
{
	return 0;
}
#endif
...
u32 engine_service_wb_monitor(struct xdma_engine *engine,
		u32 expected_wb)
{
	struct xdma_poll_wb *wb_data;
	u32 desc_wb = 0;
...
int xdma_config(struct xdma_dev *lro, unsigned int config_offset)
{
		...
		int non_incr_addr, int force_new_desc, int userspace)
{
	...
	if (userspace) {
		...
		dbg_sg("sgm_get_user_pages()\n");
	}
	else {
		rc = sgm_kernel_pages(transfer->sgm, start, cnt, !dir_to_dev);
		dbg_sg("sgm_kernel_pages()\n");
		dbg_sg("sgm_kernel_pages() rc 0x%x\n", rc);
...
*/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,15,0)
ssize_t sg_read_iter (struct kiocb *iocb, struct iov_iter *from)
{
	dbg_sg("%s()\n", __func__);
...
ssize_t sg_write_iter (struct kiocb *iocb, struct iov_iter *iter)
{
	dbg_sg("%s()\n", __func__);
		...
		ssize_t remaining, loff_t *pos, int seq)
{
	...
	while ((res == 0) && (remaining > 0)) {
		...
		transfer = transfer_create(lro, transfer_addr, transfer_len,
			*pos, engine->dir_to_dev, engine->non_incr_addr, 0, 0);
...
#else
void __devinit enable_pcie_relaxed_ordering(struct pci_dev *dev)
{
	u16 v;
	int pos;
...
void pcie_check_extended_tag(struct xdma_dev *lro, struct pci_dev *pdev)
{
	u16 cap;
	u32 v;
	void *__iomem reg;
		...
		struct xdma_engine *engine, enum chardev_type type)
{
	...
	dbg_init("setting engine %p \n", engine);
	dbg_init("setting lro  %p \n", lro);
	dbg_init("on LRO_CHAR  %p \n", lro_char);
