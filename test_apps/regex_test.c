...
unsigned long hls_write_ctl_axi_addr = 0xC0030000;
unsigned long hls_write_axi_addr = 0xC0020000;
unsigned long hls_read_ctl_axi_addr = 0x80040000;
unsigned long hls_read_axi_addr = 0x80030000;

unsigned long hls_write_ctl_2_axi_addr = 0x80020000;
unsigned long hls_write_2_axi_addr = 0x80010000;
...
int main()
{
	...
	if(ioctl(hls_write_2, SET_AXI_CTL_DEVICE, &hls_write_ctl_2_axi_addr) < 0) {
		printf("ERROR doing ioctl\n");
		return -1;
	}
	printf("set peripheral to axi base address %x\n", hls_write_ctl_axi_addr);
...
/************************* AXI STREAMING FIFO TEST ***************************************/
	...
	ret_val = write(hls_write_2, packet_binary, sizeof(packet_binary));   
	if (ret_val == 0)
		printf("WRITE ERROR\n");

	printf("TX FIFO 2: Transmitted data\n");
